...
static bm_algo_q31_t div_q31(bm_algo_q31_t num, bm_algo_q31_t den) {
    int64_t scaled;

    if (den == 0) {
        return 0;
    }
    scaled = ((int64_t)num << 31) / (int64_t)den;
...
bm_algo_q31_t bm_algo_pi_q31_step(bm_algo_pi_q31_state_t *state,
    ...
    if (config->ki != 0 && u_sat != u_unsat) {
        state->integrator = div_q31(u_sat - p_term, config->ki);
        state->integrator = bm_algo_clamp_q31(state->integrator,
                                              config->integrator_min,
                                              config->integrator_max);
    }
...
bm_algo_q15_t bm_algo_lpf1_q15_step(bm_algo_lpf1_q15_state_t *state,
    ...
    state->output = (bm_algo_q15_t)bm_algo_clamp_q15((bm_algo_q15_t)y,
                                                     (bm_algo_q15_t)(-32768),
                                                     BM_ALGO_Q15_ONE);
    return state->output;
}
