...
bm_ticker_init(slots)     /* 槽内指定 event_type、period_ms；运行中须先 reset */
主循环:
    bm_ticker_poll()
...
示例（`hrt_servo_stub`）：先 `ctrl_start_all`（设定 HRT 频率），再 `ticker_init`；`{ 100ms, EVENT_POSITION }` → `poll` 入队 → `on_position_event` 在 `process` 中执行。队列满时 ticker 推进下一周期并累计 `bm_ticker_get_dropped()`。
