...
示例（`hrt_servo_stub`）：先 `bm_ctrl_start_all`（内部再 `bm_hrt_start`），再 `bm_ticker_init`；`{ 100ms, EVENT_POSITION }` → `poll` 入队 → `on_position_event` 在 `process` 中执行。队列满时 ticker 推进下一周期并累计 `bm_ticker_get_dropped()`（临界区内读取）。
...
模块 `start` 里可注册事件、启动采样定时逻辑；运行期协作仍靠 `bm_event`。
